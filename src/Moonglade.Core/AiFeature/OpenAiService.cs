...
namespace MoongladePure.Core.AiFeature
{
    public class OpenAiService
    {
        ...
        private const string _prompt = "你是一个文章评论员。下面有一篇博客，你需要阅读这篇博客，对其中的内容进行评论。你的评论尽可能要客观详实，精准的总结博客的内容，找出其中的优点和缺点，找到其核心理念，对核心理念进行鼓励或反对。你需要找到博客最大的闪光点进行赞赏，也需要找到可以改进的地方，重复的引用，错误的语法等。详细的描述这个博客的改进空间。你的回复会直接发送给博客的作者，因此请尽可能鼓励和肯定作者的写作。你的评论需要和博文的语言相同，例如：如果博文是中文，使用中文评论。如果博文是英文，则使用英文进行评论。下面是你要评论的文章内容：";
