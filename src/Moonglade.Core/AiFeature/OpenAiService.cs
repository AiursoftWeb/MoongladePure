    ...
    IConfiguration configuration)
{
    ...
    private const string WorkPrompt = "好了，根据上面的文章，现在开始你的评论工作吧！别忘了，不要重复输出文章内容，只写出一则恰当的博客回复。（无需问候和署名，不要分段，不要使用标题）";
    ...
    private const string WorkAbstractPrompt = "好了，根据上面的文章，现在开始你的摘要工作吧！别忘了，无需问候和署名。**不要**使用markdown！**不要**分段！！！你是在做摘要而不要重新反复复述文章的内容。只输出写好的摘要！！！不要输出其它内容。不要强调你的摘要的特点。原本的博客文章如下：";
